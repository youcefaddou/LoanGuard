...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Pour le RES : bankId sera undefined (c'est normal, il peut changer de banque)
    // Pour le CHG : on récupère son bankId unique
    let userBankId = undefined;
    if (user.role === "CHG") {
      const userBanks = await prisma.userBank.findFirst({
        where: { userId: user.id },
      });
      userBankId = userBanks?.bankId;
    }
    ...
    if (user.role === "CHG") {
      ...
      if (userBanks.length === 0) {
        return res.status(403).json({
          message:
            "Aucune agence assignée à ce chargé de risques. Contactez l'administrateur",
        });
      }
      ...
      if (userBanks.length > 1) {
        return res.status(403).json({
          message:
            "Erreur configuration : un chargé de risques ne peut être assigné qu'à une seule agence",
        });
      }
      ...
      if (userBanks.length === 0) {
        return res.status(403).json({
          message:
            "Aucune agence assignée à ce responsable. Contactez l'administrateur",
        });
      }
...
exports.selectBank = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la sélection de banque" });
  }
};
